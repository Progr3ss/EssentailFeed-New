...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_deliversErrorsOnNon200HTTPResonse() {
        let (sut, client) = makeSUT()
    ...
    private func expect(_ sut: RemoteFeedLoader, toCompleteWithResult expectedResult: RemoteFeedLoader.Result, when action: () -> Void, file : StaticString = #file, line: UInt = #line) {
        ...
        sut.load { receivedResult in
            ...
            case let (.failure(receivedResult as RemoteFeedLoader.Error), .failure(expectedResult)):
                XCTAssertEqual(receivedResult, expectedResult as! RemoteFeedLoader.Error, file: file, line: line)
            default:
